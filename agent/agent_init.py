...
def init_agent(
    ...
    try:
        ...
        agent._api_retry_base_delay = max(float(_raw_retry_base), 0.0)
    except (TypeError, ValueError):
        agent._api_retry_base_delay = 3.0
    try:
        _raw_retry_multiplier = _agent_section.get("api_retry_multiplier", 1.8)
        agent._api_retry_multiplier = max(float(_raw_retry_multiplier), 1.0)
    except (TypeError, ValueError):
        agent._api_retry_multiplier = 1.8
    try:
        _raw_retry_max_delay = _agent_section.get("api_retry_max_delay", 90.0)
        agent._api_retry_max_delay = max(float(_raw_retry_max_delay), 0.0)
    except (TypeError, ValueError):
        agent._api_retry_max_delay = 90.0
